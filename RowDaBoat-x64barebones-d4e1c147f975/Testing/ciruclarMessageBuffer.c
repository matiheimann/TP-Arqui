...
{
  if ((messageBuffer->front == 0 &&
       messageBuffer->rear == messageBuffer->size - 1) ||
      (messageBuffer->rear ==
       (messageBuffer->front - 1) % (messageBuffer->size - 1)))
  {
    // queue is full !
    return;
  }
  ...
  else if (messageBuffer->rear == messageBuffer->size - 1 &&
