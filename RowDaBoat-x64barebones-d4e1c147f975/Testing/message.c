...
{
	// mutex.lock()  es el mismo mutex que va a utilizar el proceso que
	// recive el
	// mensaje
	...
	if (isblocked(msg.targetPID)) {
		awake(msg.targetPID); // si el proceso destino esta bloqueado,
				      // hay que
				      // desperarlo
	}
...
message asynchronousReceive()
{
	...
	if (messageBuffer.size == 0) {
		block(current_process); // se bloquea si no hay nada que esperar
					// (busy
					// waiting?)
	}
	tmp = pop(buff);
	while (topwaitqueue() != NULL) {
		awake(popwaitqueue()); // despertar el proceso que estaba
				       // esperando para
				       // enviar
	}
	// mutex.unlock();
...
circularMsgBuffer map_buffer(char *targetPID)
{
	...
	// hay que recorrer todos los processo hasta encontrar el que tenga el
	// mismo
	// PID,
