	...
	while (position < MEMORY_SIZE) {
		...
		if (orderOfBlockRequired <= currentBlockMetadata->order) {
			if (currentBlockMetadata->isAllocated) {
				position = jumpNextPosition(
				    position, currentBlockMetadata->order);
			} else if (orderOfBlockRequired ==
				   currentBlockMetadata->order) {
				memory[position]++; // Allocated bit set to 1
				position++; // Position to return after metadata
					    // block
			...
			position =
			    jumpNextPosition(position, orderOfBlockRequired);
		}
	}
...
void deallocate(void *address)
{
	if ((char *)address <= memory ||
	    ((char *)address > memory + MEMORY_SIZE)) {
		printf("Error, invalid memory adress\n");
		return;
	}

	int position = address - (void *)memory -
		       1; // Metada block from corresponding adress

	if (position % SMALLEST_BLOCK_SIZE != 0 ||
	    memory[position] % 2 ==
		0) // Check if is a valid metadata block position
	...
	while (position >= 0 && position < MEMORY_SIZE) {
		...
		if (metadata1->isLeft) {
			int positionBuddy =
			    jumpNextPosition(position, metadata1->order);
			...
			int positionBuddy =
			    jumpPreviousPosition(position, metadata1->order);
...
{
	int buddy =
	    jumpNextPosition(position, positionBlockMetadata->order - 1);
...
{
	return (position % (int)(SMALLEST_BLOCK_SIZE * pows(2, order + 1)) == 0)
		   ? 1
		   : 0;
}
