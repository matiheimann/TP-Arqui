	...
	if (strcasecmp(s_list, command) == 0) {
		...
		if (filename == NULL) {
			...
			if (argc > 4) {
				...
				fgets(tempstring, 32, stdin); // Get up to 32
							      // chars from the
							      // keyboard
...
void create(char *filename, unsigned long long maxsize)
{
	...
	if (findfile(filename, &tempentry, &slot) == 0) {
		...
		for (tint = 0; tint < 64; tint++) {
			pEntry = (struct BMFSEntry *)(dir_copy +
						      tint * 64); // points to
								  // the current
								  // directory
								  // entry
			...
			{
				if (first_free_entry == -1)
					first_free_entry = tint; // will use it
								 // for our new
								 // file
			}
		}
		...
		for (tint = 0; tint < num_used_entries + 1; tint++) {
			...
			// and the beginning of the current file (or the last
			// data block if
