...
static void zero_division(uint64_t rsp, uint64_t rax, uint64_t rbx, uint64_t rcx, uint64_t rdx, uint64_t rsi, uint64_t rdi) {
	printString("Zero division exception: \n");
	printExceptionStackFrame(rsp);
	printRegisters(rax, rbx, rcx, rdx, rsi, rdi);
}

static void overflow(uint64_t rsp, uint64_t rax, uint64_t rbx, uint64_t rcx, uint64_t rdx, uint64_t rsi, uint64_t rdi) {
	printString("Overflow exception: \n");
	printExceptionStackFrame(rsp);
	printRegisters(rax, rbx, rcx, rdx, rsi, rdi);
}

static void invalidOpCode(uint64_t rsp, uint64_t rax, uint64_t rbx, uint64_t rcx, uint64_t rdx, uint64_t  rsi, uint64_t rdi) {
