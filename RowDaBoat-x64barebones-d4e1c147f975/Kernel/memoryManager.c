...
void deallocate(void* address)
{
  ...
  int position =
    address - (void*)memory - 1; // Metada block from corresponding adress

  if (position % SMALLEST_BLOCK_SIZE != 0 ||
      memory[position] % 2 == 0) // Check if is a valid metadata block position
...
{
  return (position % (int)(SMALLEST_BLOCK_SIZE * pow(2, order + 1)) == 0) ? 1
                                                                          : 0;
}
