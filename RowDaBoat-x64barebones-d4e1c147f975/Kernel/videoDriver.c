...
void putpixel(int x, int y)
{
  if (validPixelPosition(x, y))
  {
    ...
    uint8_t* position =
      (uint8_t*)(initialPosition + (x + y * vesaInfo->XResolution) * 3);
    position[0] = 0xFF;
    position[1] = 0xFF;
    position[2] = 0xFF;
  }
...
{
  uint8_t* position = (uint8_t*)((uint8_t*)((uint64_t)vesaInfo->PhysBasePtr) +
...
void endScreen()
{
  ...
  for (int i = 0; i < (vesaInfo->YResolution - CHAR_HEIGHT); i++)
  {
    ...
    {
      *((uint8_t*)(initialPosition) + i * charsPerLine * CHAR_WIDTH * 3 + j) =
        *((uint8_t*)(initialPosition) +
          (i + CHAR_HEIGHT) * charsPerLine * CHAR_WIDTH * 3 + j);
    }
  }
  for (int i = vesaInfo->YResolution - CHAR_HEIGHT; i < vesaInfo->YResolution;
       i++)
  {
...
void erase()
{
  ...
  for (int i = 0; i < CHAR_HEIGHT; i++)
  {
    ...
    {
      putBlackPixel((videoCursor % charsPerLine) * CHAR_WIDTH + j,
