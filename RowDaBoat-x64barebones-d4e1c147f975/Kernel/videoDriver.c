...
static const VBEModeInfoBlock *vesaInfo =
    (VBEModeInfoBlock *)0x0000000000005C00;
...
void putpixel(int x, int y)
{
	if (validPixelPosition(x, y)) {
		uint8_t *initialPosition =
		    (uint8_t *)((uint64_t)vesaInfo->PhysBasePtr);
		uint8_t *position =
		    (uint8_t *)(initialPosition +
				(x + y * vesaInfo->XResolution) * 3);
		position[0] = 0xFF;
		position[1] = 0xFF;
		position[2] = 0xFF;
	}
...
{
	uint8_t *position =
	    (uint8_t *)((uint8_t *)((uint64_t)vesaInfo->PhysBasePtr) +
			(x + y * vesaInfo->XResolution) * 3);
	position[0] = 0x00;
	position[1] = 0x00;
	position[2] = 0x00;
...
{
	if (x < vesaInfo->XResolution && y < vesaInfo->YResolution && x >= 0 &&
	    y >= 0)
		return 1;
...
{
	if (x >= 0 && y >= 0 && x < vesaInfo->XResolution / CHAR_WIDTH &&
	    y < vesaInfo->YResolution / CHAR_HEIGHT)
		return 1;
...
void printChar(char c)
{
	...
	if (c == '\n') {
		...
		for (int i = 0; i < CHAR_HEIGHT; i++) {
			...
			printCharLine(pixel, x * CHAR_WIDTH,
				      (y * CHAR_HEIGHT + i));
		}
		videoCursor++;
	}
...
void endScreen()
{
	...
	for (int i = 0; i < (vesaInfo->YResolution - CHAR_HEIGHT); i++) {
		for (int j = 0; j < (vesaInfo->XResolution) * 3; j++) {
			*((uint8_t *)(initialPosition) +
			  i * charsPerLine * CHAR_WIDTH * 3 + j) =
			    *((uint8_t *)(initialPosition) +
			      (i + CHAR_HEIGHT) * charsPerLine * CHAR_WIDTH *
				  3 +
			      j);
		}
	}
	for (int i = vesaInfo->YResolution - CHAR_HEIGHT;
	     i < vesaInfo->YResolution; i++) {
...
void erase()
{
	...
	for (int i = 0; i < CHAR_HEIGHT; i++) {
		for (int j = 0; j < CHAR_WIDTH; j++) {
			putBlackPixel(
			    (videoCursor % charsPerLine) * CHAR_WIDTH + j,
			    (videoCursor / charsPerLine) * CHAR_HEIGHT + i);
		}
	}
}

uint16_t getXres() { return vesaInfo->XResolution; }

uint16_t getYres() { return vesaInfo->YResolution; }
